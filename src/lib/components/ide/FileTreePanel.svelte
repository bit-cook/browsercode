...
{#snippet treeNode(nodes: TreeNode[], depth: number, parentPath: string)}
	{#if pendingCreate && pendingCreate.parent === parentPath}
		{@render nameInput(
			0.75 + depth * 0.75,
			pendingCreate.kind === 'folder' ? 'catppuccin:folder' : fileIcon(actionName),
			commitCreate
		)}
	{/if}
	{#each nodes as node (node.path)}
		{#if node.children}
			{#if renaming?.path === node.path}
				{@render nameInput(0.5 + depth * 0.75, 'catppuccin:folder', commitRename)}
			{:else}
				<button
		...
		{:else if renaming?.path === node.path}
			{@render nameInput(0.75 + depth * 0.75, fileIcon(actionName), commitRename)}
		{:else}
			<button
